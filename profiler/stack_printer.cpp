...
static void print_fold_lua_func(const struct syms *syms, const struct lua_stack_event *eventp)
{
    ...
    if (eventp->type == FUNC_TYPE_LC) {
        if (eventp->line) {
            printf(";LC:%s:%d", eventp->name, eventp->line);
        } else {
            printf(";LC:%s", eventp->name);
        }
    } else if (eventp->type == FUNC_TYPE_CC) {
        const struct sym *sym = syms__map_addr(syms, (unsigned long)eventp->funcp);
        if (sym)
        {
            printf(";CC:%s", sym ? sym->name : "[unknown]");
        } else {
            printf(";CC:%p", eventp->funcp);
        }
    } else if (eventp->type == FUNC_TYPE_CF) {
        const struct sym *sym = syms__map_addr(syms, (unsigned long)eventp->funcp);
        if (sym)
        {
            printf(";CF:%s", sym ? sym->name : "[unknown]");
        } else {
            printf(";CF#%p", eventp->funcp);
        }
    } else {
        printf(";[unknown]");
    }
...
static void print_fold_user_stack_with_lua(const lua_stack_backtrace *lua_bt, const struct syms *syms, unsigned long *uip, unsigned int nr_uip)
{
	...
	// fprintf(stderr, "print_fold_user_stack_with_lua ===== [%d]\n", lua_bt_count);
